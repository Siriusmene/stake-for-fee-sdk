...
export class StakeForFee {
  constructor(
    ...
    public xVault: DynamicVault,
    public yVault: DynamicVault,
  ...
  public static async createFeeVault(
    ...
    const tokenXVaultKey = getAssociatedTokenAddressSync(
      poolState.tokenAMint,
      feeVaultKey,
      true
    );
    const tokenYVaultKey = getAssociatedTokenAddressSync(
    ...
    const transaction = await stakeForFeeProgram.methods
      ...
      .accounts({
        ...
        pool,
        tokenXVault: tokenXVaultKey,
        tokenYVault: tokenYVaultKey,
  ...
  public async claimFee(owner: PublicKey): Promise<Transaction> {
    ...
    const [
      { ataPubKey: userTokenX, ix: initializeUserTokenXIx },
      { ataPubKey: userTokenY, ix: initializeUserTokenYIx },
    ] = await Promise.all([
      getOrCreateATAInstruction(
        this.connection,
    ...
    initializeUserTokenXIx && preInstructions.push(initializeUserTokenXIx);
    initializeUserTokenYIx && preInstructions.push(initializeUserTokenYIx);
